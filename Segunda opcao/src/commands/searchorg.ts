...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try {
    ...
    if (!orgs.length) { await interaction.editReply('No orgs found for that query.'); return; }
    const org = orgs[0]!;
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: 'Region', value: org.region || 'â€”', inline: true },
        { name: 'Status', value: org.status?.toUpperCase() || 'â€”', inline: true },
        { name: 'Founded', value: org.founded || 'â€”', inline: true },
        { name: 'Record', value: `${org.wins}W â€” ${org.losses}L`, inline: true },
        { name: 'Winrate', value: wr, inline: true },
        { name: 'Members', value: String(org.members?.length || 0), inline: true },
        { name: 'MVP', value: org.mvp || 'â€”', inline: true },
