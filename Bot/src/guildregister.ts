...
export async function execute(
  ...
  try {
    ...
    const canCreateGuild = isAdmin || (!!actorMember && (
      configuredRegisterRole
    ...
    db.prepare(
      `INSERT INTO Guilds (id, name, leaderId, coLeaderId, imageUrl, region)
       VALUES (?, ?, ?, ?, ?, ?)`
    ...
    if (managersFromDb.length > 0) {
      ...
      description += managersFromDb.map((m: any) => `<@${m.userId}>`).join(' ') + '\n\n';
    } else {
      description += `**Managers**\nNone\n\n`;
    }
    ...
    if (mainsFromDb.length > 0) {
      description += mainsFromDb.map((m: any) => `<@${m.userId}>`).join('\n') + '\n\n';
    } else {
      description += 'None\n\n';
    }
    ...
    if (subsFromDb.length > 0) {
      description += subsFromDb.map((s: any) => `<@${s.userId}>`).join('\n');
    } else {
      description += 'None';
    }
    ...
    const channel = (await interaction.client.channels.fetch(channelId).catch(() => null)) as any;
    ...
    if (channel && channel.type === ChannelType.GuildForum) {
      try {
        ...
        db.prepare('UPDATE Guilds SET panelMessageId = ?, panelChannelId = ? WHERE id = ?').run(
          thread.id,
          channel.id,
          guildUid
        );
