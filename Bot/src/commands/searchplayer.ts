...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(0x5BADFF)
      .addFields(
        { name: 'ELO', value: String(p.elo || 1000), inline: true },
        { name: 'Wins', value: String(p.wins || 0), inline: true },
        { name: 'Losses', value: String(p.losses || 0), inline: true },
        { name: 'Winrate', value: wr, inline: true },
        { name: 'Guild', value: p.org || 'Free Agent', inline: true },
      );
