...
export async function execute(interaction) {
    ...
    try {
        ...
        const org = orgs[0];
        const wr = (org.wins + org.losses) > 0 ? `${((org.wins / (org.wins + org.losses)) * 100).toFixed(0)}%` : '—';
        const embed = new EmbedBuilder()
            .setTitle(`${org.name} [${org.tag}]`)
            .setColor(0x5BADFF)
            .setThumbnail(org.logo_url || null)
            .addFields({ name: 'Region', value: org.region || '—', inline: true }, { name: 'Status', value: org.status?.toUpperCase() || '—', inline: true }, { name: 'Founded', value: org.founded || '—', inline: true }, { name: 'Record', value: `${org.wins}W — ${org.losses}L`, inline: true }, { name: 'Winrate', value: wr, inline: true }, { name: 'Members', value: String(org.members?.length || 0), inline: true }, { name: 'MVP', value: org.mvp || '—', inline: true }, { name: 'Points', value: String(org.points || 0), inline: true });
